...
const configureForm =  async (userContactDoc) => {
...
const load =  async () => {
  ...
  await browser.pause(1000); // let the refresh work here - #3691

  // select form
  const addButton = await $('.action-container .general-actions:not(.ng-hide) .fa-plus');
  await addButton.click();
  const form = await $('.action-container .general-actions .dropup.open .dropdown-menu li:last-child a');
  await form.click();
  const female = await $('[name="/data/my_sex"][value="female"]');
  return await female.waitForDisplayed();
};

const reset  = async () => {
  const refreshIcon = await $('.icon.icon-refresh');
  await refreshIcon.click();
};

const setHeight =  async (height) => await clearAndFill((`[name="/data/my_height"]`), height);
const setWeight =  async (weight) => await clearAndFill((`[name="/data/my_weight"]`), weight);
const setAge =  async (age) => await clearAndFill((`[name="/data/my_age"]`), age);
const setSex =  async (sex) => {
  const sexBox = await $(`[name="/data/my_sex"][value="${sex}"]`);
  await sexBox.click();
};

const setPatient =  async (patient) => {
  await setSex(patient.sex);
  await setAge(patient.age);
  await setHeight(patient.height);
  await setWeight(patient.weight);
};

const getHeightForAge = async () => await clickAndGetValue('[name="/data/hfa"]');
const getWeightForAge = async () => await clickAndGetValue('[name="/data/wfa"]');
const getWeightForHeight = async () => await clickAndGetValue('[name="/data/wfh"]');
const fieldByIndex = async (index) => {
  const field = await $(`#reports-content .details li:nth-child(${index}) p`);
